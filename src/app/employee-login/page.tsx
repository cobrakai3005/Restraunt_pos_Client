...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Store, UserCircle, KeyRound, ArrowRight, ShieldCheck, Delete } from "lucide-react";
import { ThemeToggle } from "@/components/layout/theme-toggle";
import Link from "next/link";
...
/**
 * On-screen numeric dial pad for POS PIN entry.
 * Mirrors the existing theme (blue-600 accents, slate surfaces, dark mode support).
 */
function DialPad({
  value,
  onChange,
      ...
      <div className="grid grid-cols-3 gap-3">
        {keys.map((k) => (
          <button
            ...
            className="h-14 rounded-lg text-xl font-semibold bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white hover:bg-blue-50 dark:hover:bg-blue-500/10 hover:border-blue-300 dark:hover:border-blue-500/40 active:scale-95 transition-all"
          >
            {k}
          </button>
        ))}
        ...
        <button
          type="button"
          onClick={handleClear}
          className="h-14 rounded-lg text-xs font-semibold uppercase tracking-wide bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:bg-red-50 dark:hover:bg-red-500/10 hover:border-red-300 dark:hover:border-red-500/40 hover:text-red-600 dark:hover:text-red-400 active:scale-95 transition-all"
        >
          Clear
        </button>
        ...
        <button
          ...
          className="h-14 rounded-lg flex items-center justify-center bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:bg-blue-50 dark:hover:bg-blue-500/10 hover:border-blue-300 dark:hover:border-blue-500/40 hover:text-blue-600 dark:hover:text-blue-400 active:scale-95 transition-all"
        >
...
function EmployeeLoginForm() {
  ...
  // Always synchronize Restaurant ID from URL or localStorage upon client mount
  useEffect(() => {
    const fromUrl = searchParams.get("restaurantId");
    const fromStorage = typeof window !== "undefined" ? localStorage.getItem("vinimay_restaurant_id") : null;
    const idToUse = fromUrl || fromStorage;
    if (idToUse) {
      pinForm.setValue("restaurantId", idToUse, { shouldValidate: true });
    }
  }, [searchParams, pinForm]);

  const handleLoginSuccess = (token: string, user: User, restaurantId?: string) => {
    login(token, user);
    const idToSave = restaurantId || user?.restaurantId;
    if (idToSave && typeof window !== "undefined") {
      localStorage.setItem("vinimay_restaurant_id", idToSave);
    }
    toast({
      title: "Login Successful",
      description: "Welcome to your terminal.",
              ...
              <TabsContent value="pin" className="mt-0">
                <Form {...pinForm}>
                  <form onSubmit={pinForm.handleSubmit(onPinSubmit)} className="space-y-6">
                    ...
                    {/* POS PIN - dial pad entry replaces the plain text input */}
                    <FormField
                      control={pinForm.control}
                      name="posPin"
                      render={({ field }) => (
                        <FormItem>
                          ...
                          <FormControl>
                            <div>
                              {/* Hidden input keeps react-hook-form wiring/ref intact */}
