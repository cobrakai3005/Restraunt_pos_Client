...
interface EditAdminRestaurantDialogProps {
  open: boolean;
...
export function EditAdminRestaurantDialog({ open, onOpenChange, restaurant, onSuccess }: EditAdminRestaurantDialogProps) {
    ...
    if (restaurant && open) {
      setTimeout(() => {
        form.reset({
          ...
          clientId: typeof restaurant.clientId === 'object' ? restaurant.clientId._id : (restaurant.clientId || ""),
          ...
          contact: {
            ...
            managerName: restaurant.contact?.managerName || ""
          },
          compliance: {
            gstNumber: restaurant.compliance?.gstNumber || "",
            fssaiNumber: restaurant.compliance?.fssaiNumber || ""
          }
        });
      }, 0);
    }
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      await adminService.updateRestaurant(restaurant._id, values);
      toast({ title: "Success", description: "Restaurant updated successfully." });
      onSuccess();
      onOpenChange(false);
    } catch (error: any) {
      toast({ variant: "destructive", title: "Error", description: error.response?.data?.message || "Failed to update restaurant" });
    } finally {
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-6 text-white flex items-center gap-4">
          ...
          <div>
            ...
            <DialogDescription className="text-blue-100 text-sm mt-1">
              Update details for this restaurant branch.
            </DialogDescription>
          </div>
        </div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                ...
                name="isActive"
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-2 px-4 bg-card text-card-foreground">
                    <div className="space-y-0.5">
                      <FormLabel className="text-sm text-slate-700 dark:text-slate-300">Active Status</FormLabel>
                    </div>
                    <FormControl>
