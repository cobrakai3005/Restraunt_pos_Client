...
export function MenuItemCard({
  ...
  return (
    ...
    <Card
  className={`group cursor-pointer transition-all duration-300 flex flex-col overflow-hidden active:scale-[0.97]
    ...
    ${
      inCartCount > 0
        ? "ring-2 ring-blue-500/80 bg-blue-50/40 dark:bg-blue-900/20"
        : ""
    }
  `}
  onClick={onClick}
>
  ...
  <div className="relative h-12 shrink-0 overflow-hidden bg-slate-100 dark:bg-slate-900">
    ...
    {/* Short Code - Top Left */}
    {shortCode && (
      <div className="absolute left-2 top-2 z-20 flex items-center justify-center rounded-md border border-slate-700/60 bg-slate-950/80 px-1.5 py-0.5 text-[9px] font-mono font-extrabold uppercase text-amber-300 shadow-sm backdrop-blur-sm">
        {shortCode}
      </div>
    )}
    ...
    {/* Variants - Top Right */}
    {variantsCount > 1 && (
      <div className="absolute right-2 top-2 z-20 flex items-center gap-1 rounded-md border border-violet-300/40 bg-violet-600/90 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-white shadow-md backdrop-blur-sm">
        <Layers className="h-3 w-3" />
        {variantsCount} Variants
      </div>
    )}

    {/* Cart Count - Bottom Right */}
    {inCartCount > 0 && (
      <div className="absolute bottom-1.5 right-2 z-20 flex h-6 min-w-6 items-center justify-center rounded-full bg-blue-600 px-1.5 text-xs font-extrabold text-white shadow-md animate-in zoom-in-50">
        {inCartCount}
      </div>
    )}
  ...
  <CardContent className="relative flex flex-1 flex-col p-3 pt-2.5">
    ...
    <div className="flex items-start gap-1.5">
      ...
      <h3 className="line-clamp-2 pr-2 text-[13px] font-bold leading-tight text-slate-900 dark:text-white">
        {name} 
      </h3>
      <span className="font-mono text-lg font-black tracking-tight text-blue-600 dark:text-blue-400">
        ₹{price.toFixed(2)}
      </span>
    </div>
